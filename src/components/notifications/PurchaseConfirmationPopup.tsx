  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50 p-2 sm:p-4">
      <div className="bg-[var(--card)] p-4 sm:p-6 rounded-lg shadow-xl w-full max-w-xs sm:max-w-sm relative">
        ...
        <h2 className="text-sm font-bold text-white mb-2">How many {tokenSymbol} do you want to buy? - amount in {getTokenSymbol()}</h2>
        <p className="text-[10px] sm:text-xs text-gray-400 mb-3 italic leading-tight">
          ...
          When creating, creators can buy up to 5% of the trading supply; any excess {getTokenSymbol()} is refunded.
        </p>
        <input
          type="number"
        ...
        <p className="text-[8px] sm:text-xs text-gray-400 text-center">
          Cost to deploy: ~0 {getTokenSymbol()}
        </p>
      </div>
    </div>
  );
