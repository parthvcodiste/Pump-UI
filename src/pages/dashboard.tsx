...
const TokenBalanceItem: React.FC<{
  ...
  const handleAddressClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    window.open(
      `${process.env.NEXT_PUBLIC_EXPLORER_URL}/address/${tokenAddress}`,
      "_blank"
    );
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    >
                      {token.logo && (
                        <img
                          src={token.logo}
                          alt={`${token.name} logo`}
                          className="w-16 h-16 mr-3 sm:mr-4 rounded-full"
                        />
                      )}
                      <div>
        ...
        <div>
          ...
          {isLoading ? (
            ...
            <div className="overflow-x-auto bg-gray-800 rounded-lg shadow-md">
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-gray-900">
                  <tr>
                    <th className="px-4 py-3 text-left text-[10px] sm:text-xs font-medium text-gray-300 uppercase tracking-wider">
                      Type
                    </th>
                    <th className="px-4 py-3 text-left text-[10px] sm:text-xs font-medium text-gray-300 uppercase tracking-wider">
                      Token
                ...
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {transactions.map((tx) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 whitespace-nowrap text-[10px] sm:text-xs text-gray-300">
                        {getCustomTokenSymbol(tx.recipientAddress)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-[10px] sm:text-xs text-gray-300">
                        {formatAmountV3(tx.tokenAmount)}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-[10px] sm:text-xs text-gray-300">
                        {formatAmountV3(tx.ethAmount)} {getTokenSymbol()}
                      </td>
                      <td className="px-4 py-3 whitespace-nowrap text-[10px] sm:text-xs text-gray-300">
            ...
            <p className="text-gray-300 bg-gray-800 rounded-lg p-4 shadow-md">
